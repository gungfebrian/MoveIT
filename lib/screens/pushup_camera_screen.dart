...
class _PushUpCameraScreenState extends State<PushUpCameraScreen> {
  ...
  Future<void> _processImage(CameraImage image) async {
    ...
    try {
      // Debug: Log image info every 50 frames
      if (_frameCount % 50 == 0) {
        debugPrint(
          '📷 Image: ${image.width}x${image.height}, planes: ${image.planes.length}',
        );
        debugPrint(
          '📐 Rotation: ${_getImageRotation()}, Camera: ${_currentCamera.lensDirection}',
        );
      }
      ...
      if (_frameCount % 50 == 0) {
        ...
        if (poses.isNotEmpty) {
          debugPrint('   Landmarks: ${poses.first.landmarks.length}');
        }
      }
